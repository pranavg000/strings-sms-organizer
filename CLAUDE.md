...
- **Date section headers.** Message lists use `dateSectionLabel` + `DateSeparator` (`ui/components/MessageList.kt`: Today / Yesterday / This week / month). Finance ledgers (Overview tab, Account detail) share `LazyListScope.transactionItems` (`ui/finance/TransactionListItems.kt`), which reuses `DateSeparator` with `transactionDateSectionLabel` (Today / Yesterday / then one header per day, e.g. "Mon, 1 Sep") because those lists are already month-scoped; rows then show time only (`formatTransactionClock`). Both use the same adjacent-item label comparison -- don't precompute groups.
...
- **Per-message transaction override (`Message.isTransactionExcluded`).** A user can flip detection for one message via "Not a transaction" (message detail overflow + ledger row overflow, with an Undo snackbar in finance) and "Mark as transaction" (detail overflow when no transaction exists). `ToggleTransactionUseCase.exclude()` persists the flag, deletes the transaction, and strips the Finance-family tags (`TransactionCategorizer.clearTransaction`, the inverse of a match); `include()` clears the flag and re-runs the *normal* `categorize()` -- no bespoke parsing -- returning `IncludeTransactionResult` so the UI can toast when nothing was recognized. `categorize()` short-circuits on the flag (after the delete), so batch recategorization never resurrects an excluded message. The flag is user-curated, so it rides in `MessageStateDto.isTransactionExcluded` (backup v5) and import applies it after the tag set restore.
